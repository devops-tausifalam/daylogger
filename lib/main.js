...
const unsplash = `https://api.unsplash.com/photos/random?&topics=${imageTopics}&query=dark&orientation=landscape&count=1&client_id=${unsplashAPI}`;
const demo1 = `http://127.0.0.1:3000/lib/config/test-unsplash-response.json`;
...
async function loadunsplash() {
    try {
        ...
        const unsplash_resp = await fetch(demo1)
        if (!unsplash_resp.ok) {
...
const demo2 = `http://127.0.0.1:3000/lib/config/test-quote.json`;
async function loadQuote() {
    try {
        const quote_resp = await fetch(demo2);
        if (!quote_resp.ok){
