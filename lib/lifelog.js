...
userTxtInput.addEventListener('blur', (e) => {
...
function saveData() {
    ...
    let hour = getTime.getHours().toLocaleString();
    let minutes = getTime.getMinutes().toLocaleString();
    const data = {
        blurt_timestamp: `${hour}:${minutes}`,
    ...
    const entryHTML = `
        <li>
            <div class="log-entry">
                <span class="chatTxt">${data.blurt}</span>
                <br/>
                <span class="chatTimeStamp">${data.blurt_timestamp}</span>
            </div>
        </li>
    `;
